...
- **Subscription Refresh（少打扰）：**
  ...
  - **禁止：** 固定后台周期拉订阅（耗电、体验差）。
  - **成功：** 更新节点/规则，写 Activity；**不**弹成功打扰。
  - **失败：** **不覆盖**已有可用配置；需要时再提示/诊断（导入失败原则一致）。
  - **手动：** 用户可随时更新；Repair #2 = 失败路径重载。
  - 非 Active：不自动刷，除非切换为 Active 或手动。
...
- 测可达性、排除不可用、加权评分（非仅 Ping）、建 VPN、**Connectivity Probe 验证出网**。
- **Connection Success（验收定义）：** 系统 VPN/隧道就绪 **且** 至少一次 Connectivity Probe 成功。仅隧道就绪而探针失败 → 不宣称成功，进入 Diagnostic Engine 分桶。
- Connectivity Probe：客户端固定探针目标（实现期选定）；用于选节点评分、首次连接与 Repair 后验证；**不是**流媒体/站点解锁检测，不承诺解锁。
- Wi‑Fi / 蜂窝切换后应能恢复连接（恢复后仍以 Probe 判定是否 Success）。
- **Beta：** 含自动选节点与 **Node Failover**（全免）。目标商业草稿中 Free 仅「首次」自动连、后台切换属 Pro——见 §4.8.2，Beta 不执行。
- **Node Failover vs Repair（必须区分）：**
  - **Failover：** 已启用自动选节点且未钉节点时，为维持 Connection Success **自动**换节点/短重连；**不**走 Repair 确认；写入 Activity。
  - **Repair：** 仅诊断为 Client-Fixable 后、用户一键确认的 Allowlist 流程（其中也可换节点，但是「修」不是「保活」）。
  - 用户关闭自动选节点或钉死当前节点 → 禁止静默 Failover。
...
| **变更** | 连接/断开；切换或钉节点；Auto/Global/Direct；应用/清除少量 User Override；预设 DNS；触发诊断；**Repair**（Repair Consent）；**回滚 Snapshot**；切换 Active Subscription；手动 Subscription Refresh |
