...
在用户启用自动选节点时，为维持 Connection Success 可**自动**执行 Node Failover（改选可用节点或短重连），不经过 Repair 的一键确认。**Preferred node**（Location 点选偏好）**不**阻止 Failover：会话可暂用其他节点；偏好保留，供后续优先尝试（ADR **0055** / **0056**）。Failover 属 Node Selection / 连接保活，须记入 Activity；连续失败 **不**自动诊断（用户可 Help · ADR **0060**）。用户**关闭**自动选节点时禁止静默切换。Repair 仍仅用于诊断为 Client-Fixable 后、经用户确认的 Allowlist 流程。

**用户可见（grill · D · 2026-08-07）：** Failover **成功换节点**后 **一次短 toast**（~2–3s · `home.failover.toast`）；Home 无常驻离偏好态。非 Repair、非诊断入口。
