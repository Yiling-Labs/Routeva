...
**选节点：** 国旗 Cover Flow；**标题用节点名**（非国家名）+ **弱化协议标注**（如 `· VMess`）。**Idle：** Cover Flow 下节点名行 + 弱 › **可点 → Location**（无中部空 *Location ›* pill；中部 *Not Connected*）。**连接失败：** 回 Idle + 短 toast（**无**常驻 *Can’t connect* · ADR **0059**）。**Swipe / Connecting：** 节点名行锁定。**Connected：** 绿场中部节点行可点（见 ADR **0056**）+ Mode 次级入口。Cover Flow 横滑 = 临时焦点；Location 点选 = Preferred。
