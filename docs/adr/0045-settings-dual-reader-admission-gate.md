# Settings：双读者 C2 + 准入闸门

产品要覆盖小白与半专业（人群 C），但 **Settings Surface 不实现为双主 IA**。采用 **C2**：同一套短策略闭集；**半专业为主读者**（主动改意图）；小白为被引导的次要读者（默认 + 短副文 + 失败/Help 引进同一页）。扩大人群靠 Table Stakes Connect + Self-Healing + Help，**不**靠设置密度或 Shadowrocket 对标。
...
**拒绝：** 根页简单/高级双模式；竞品设置清单当 backlog；为「更小白」删掉 Mode/DNS/Override 可发现入口导致 Repair 状态黑盒；Settings 根页 History 审计墙（见 **0051**）。

**后果：** 见 CONTEXT **Settings Surface**、**Settings Admission Gate**；与 ADR 0021 / 0023 / 0027 / 0029 / 0032 / **0051** 一致并补读者与准入标准。
