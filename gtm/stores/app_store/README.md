# Apple App Store

> **当前阶段：** Brand Presence + 截图脊柱已锁。  
> 截图成品：[screenshots/](./screenshots/)（**A · Jobs** · 8 locale · 6.5" + 6.7"）。  
> 视觉源：[design/hi-fi/current/store-screenshots/](../../../design/hi-fi/current/store-screenshots/) · ADR **0071**。  
> 对外叙事主职见官网 + [ADR 0052](../../../docs/adr/0052-marketing-site-brand-presence.md)。

## 放什么

- App Icon、各尺寸设备截图、可选预览视频封面
- App Store 文案（名称、副标题、描述、关键词、更新说明）

| 槽位 | 路径 |
|---|---|
| Icon | [icon/](./icon/) |
| iPhone 截图 | [screenshots/](./screenshots/)（6.5" + 6.7"） |
| 提交文案 | [ios-listing.md](./ios-listing.md) |

## 制作前

阅读 [../../specs/app_store.md](../../specs/app_store.md)

## 命名建议

- `ios-icon-1024.png`
- `ios-screenshot-6_7-01.png`
- `ios-listing.md`
